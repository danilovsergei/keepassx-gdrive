...
void TestDatabaseSync::testEntryAddUpdate()
{
  ...
  QCOMPARE(groupRoot->entries().first()->attributes()->value("Title"),
           newEntry->attributes()->value("Title"));

  // verify last modification time was updated
  QCOMPARE(groupRoot->entries().first()->timeInfo().lastModificationTime(),
           newEntry->timeInfo().lastModificationTime());
...
// store all data from original entry before modification
  Entry *backupEntry = new Entry();
  backupEntry->copyDataFrom(entry);
...
// time of the entry before sync. Should go to the history item after sync
  QDateTime oldLastUpdated = entry->timeInfo().lastModificationTime();
...
// verify last modification time was updated
  QCOMPARE(entry->timeInfo().lastModificationTime().toTime_t(),
           newEntry->timeInfo().lastModificationTime().toTime_t());

  Entry *historyEntry = entry->historyItems().first();
// verify last modification time of the history entry saved correctly
  QCOMPARE(historyEntry->timeInfo().lastModificationTime().toTime_t(), oldLastUpdated.toTime_t());
// verify local item was saved in history
  QCOMPARE(historyEntry->attributes()->value("Password"),
           backupEntry->attributes()->value("Password"));
  QCOMPARE(historyEntry->title(), backupEntry->title());
