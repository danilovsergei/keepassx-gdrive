...
QMap<QueryEntry::QueryFilter,
     QString> QueryEntry::filters = QMap<QueryEntry::QueryFilter, QString>();
...
const QMap<QueryEntry::QueryFilter, QString> &QueryEntry::getFilters()
{
    if (!QueryEntry::filtersInitialized) {
        ...
        QueryEntry::filters[QueryFilter::LAST_MODIFIED] = QString(
            "properties has { key='%1' and value='%2' and visibility='PUBLIC'}").arg(LAST_MODIFIED);
    }
    return QueryEntry::filters;
}

QueryEntry::QueryEntry(const QueryFilter &entryType,
                       const QStringList &parameters) : type(entryType), parameters(parameters)
{
}
