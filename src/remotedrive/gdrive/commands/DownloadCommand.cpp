...
DownloadCommand::DownloadCommand(Session *session)
  : session(session)
{}

// template GoogleDrive::FileInfo
// KeePassxDriveSync::Command::parseOption<GoogleDrive::FileInfo>(const
// QVariantMap&, const QString&);
// template<> QString KeePassxDriveSync::Command::parseOption<QString>(const
// QVariantMap&,const QString&,const QString&);

void DownloadCommand::execute(const QVariantMap& options) {
  Q_ASSERT(options.size() >= 1);

  const FileInfo fi    = parseOption<FileInfo>(options, OPTION_FI);
  const QString  dbDir =
  ...
  qDebug() << "download thread =" << this->thread();
